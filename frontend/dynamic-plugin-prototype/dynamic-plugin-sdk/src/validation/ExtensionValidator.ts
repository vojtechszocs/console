...
const isNormalModule = (m: webpack.Module): m is webpack.NormalModule =>
  m instanceof webpack.NormalModule;

const findWebpackModules = (
  compilation: webpack.Compilation,
  exposedModules: ExposedPluginModules,
) => {
  const webpackNormalModules = Array.from(compilation.modules).filter(isNormalModule);
  return Object.keys(exposedModules).reduce((acc, moduleName) => {
    acc[moduleName] = webpackNormalModules.find((m) => m.rawRequest === exposedModules[moduleName]);
    return acc;
  }, {} as { [moduleName: string]: webpack.NormalModule });
};
